<template>
    <form class="flex flex-1 justify-center relative">
        ...
        <div class="w-full absolute top-14 bg-white shadow-md rounded-md overflow-hidden" style="z-index: 9999;" v-if="!isHidden">
            <div class="overflow-y-auto max-h-80" style="height: auto" v-if="suggests.length">
                <div class="flex items-center justify-between p-2 hover:bg-gray-200 cursor-pointer border-b" v-for="item in suggests" :key="item.id">
                    <div class="w-full lg:w-1/5 flex items-center justify-center mx-auto">
                        <img class="w-12" :src="item.product_images[0]?.imageUrl">
                    </div>
                    ...
                    <div class="w-1/5 text-red-500 text-center">
                        <span class="text-center">
                            <p class="line-through text-gray-500 text-[12px]">
                                {{ item.price | toVndCurrency }}
                            </p>
                            {{ calSalePrice(item.price, item.salePrice) | toVndCurrency }}
                        </span>
                    </div>
                </div>
            </div>
            <div v-else>
                <h2 class="p-2 bg-black text-white uppercase">Lịch sử tìm kiếm</h2>
                <div class="flex justify-around items-center p-2 border-b hover:bg-gray-200 cursor-pointer">
                    <div class="w-full lg:w-4/5">
                        <svg class="inline mr-2" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
...
export default {
    ...
    methods: {
        ...
        calSalePrice(originalPrice, salePrice) {
